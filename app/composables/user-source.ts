...
interface UseUserSourceReturn {
  saveUserSource: () => void
  getUserSource: () => Ref<Nullable<UserSource>>
}
 ...
 */
export function useUserSource(options: UseUserSourceOptions = {}): UseUserSourceReturn {
  ...
  function getUserSource(): Ref<Nullable<UserSource>> {
    const source = {
      referer: refererCookie.value,
      target: targetCookie.value,
    }
    return ref(!source.referer && !source.target ? null : source)
  }
